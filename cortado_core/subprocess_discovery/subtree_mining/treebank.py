...
class TreeBankEntry(): 
    ...
    def __init__(self, tree : ConcurrencyTree, tid : int, nTraces : int):
...
def create_treebank_from_cv_variants(variants : Mapping[Group, List[Trace]], artifical_start = False) -> Mapping[int, TreeBankEntry]:
    """
    ...
    for i, group in enumerate(sorted(list(variants.keys()), key = lambda x: len(variants[x]), reverse = True)):
        ...
        treeBank[i] = TreeBankEntry(tree, i, nTraces)
